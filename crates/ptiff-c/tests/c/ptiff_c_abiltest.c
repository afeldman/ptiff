 ...
 *
 * Compiles against the hand-maintained C headers (bindings/c headers, the source
 * of truth) and links against the ptiff-c staticlib (libptiff_c.a). It drives
 * the real pie-Rust implementation over the C ABI and verifies values, so it
 * proves the Rust core is callable from pure C with no C++ anywhere in the
 * picture.
 *
 * Coverage:
