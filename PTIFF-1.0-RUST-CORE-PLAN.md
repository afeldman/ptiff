...
| H8 | parallele Kompression: ≥ C++ sequential bei ≥2 Cores. **✅ gemessen (2026-08-24, `benchmarks/parallel_bench`): LZW-Compress ≈ 1.9×/3.7×/6.9× bei 2/4/8 Cores (nahezu linear); Deflate-Compress ≈ 1.7×/1.8×/1.1× (Sweet-Spot bei 2–4 Cores, danach Pool-/Bandbreiten-Overhead).** |
...
eigenen Haupt-Business-Case der Migration nicht. **Beides ist inzwischen nachgeholt:** Phase 5 ✅
(2026-08-24, s. Phase-5-Zeile) und die facheliche Scene-Verdrahtung von Phase 6 ✅ (2026-08-24).
...
## Phase 6 – Metadaten / wissenschaftliche Erweiterungen ✅ (Kern + Facade)
- **Status:** ✅ RFC-7002-Codec, die fünf Private-Tags 65001–65005 (Write/Read), Golden-Payloads und
  die idiomatsche Scene-Wiring sind in Rust umgesetzt (`ptiff_metadata.rs`,
  `directory_writer.rs`/`directory.rs`, `image_descriptor.rs`, `scene_serializer.rs`/`scene_deserializer.rs`).
...
- **Tests:** Metadaten-Vergleich gegen C++-Referenz; Golden-Payloads; Round-Trip aller fünf Domänen.
- **Risiko:** wenig.
- **DoD:** Rust erzeugt/liest dieselben PTIFF-Metadaten-Payloads wie C++.
- **Abgeschlossen (Detail):**
  - Alle fünf Tags (65001 Spice, 65002 Camera, 65003 CRS, 65004 Scientific-Layers, 65005 Provenance)
    werden aus den `ptiff.<domain>.<key>`-StorageModel-Feldern geschrieben/ gelesen und round-trippen
    deterministisch/byte-identisch (Golden-locked).
  - Typisierte Camera (65002) / CRS (65003) round-trippen durch `Scene`/`ImageDescriptor`.
  - **Neu:** generische `ptiff.*`-Metadaten für spice/layers/provenance + unbekannte Schlüssel
    round-trippen durch `Scene`/`Image` über `ImageDescriptor.metadata` (BTreeMap) + `Image::metadata()`;
    `ImageDescriptorBuilder::metadata(k, v)`; `validate_extension_key` schützt die reservierten
    camera/CRS-Schlüssel vor Kollision.
  - **RFC-7002-Toleranz (§3.3/§4.4):** eine teilweise/unvollständige `ptiff.camera.*`/`ptiff.crs.*`-Domäne
    (z.B. von einem Fremd-/Zukunfts-Writer) failt die Datei **nicht** mehr: typisierte Rekonstruktion wird
