...
| 4 | Kompression | 🚧 | PackBits/LZW/Predictor (dependency-frei) fertig; **Zarr-ZSTD fertig** (in `backend/zarr/codec.rs`, als Zarr-Chunk-Kompression via `zstd`/`flate2`, 2026-08-24); **Deflate/JPEG als TIFF-Codecs fehlen, ZSTD als TIFF-Codec offen** (RFC-0011) |
...
## Phase 4 – Kompression
- **Ziel:** PackBits, LZW, Predictor (eigene), Deflate (flate2), JPEG (libjpeg-turbo), Zarr-ZSTD.
- **Betroffen:** `src/compression/*` → Rust-Features.
- **Abhängigkeiten:** flate2, zstd, jpeg-Bindung.
