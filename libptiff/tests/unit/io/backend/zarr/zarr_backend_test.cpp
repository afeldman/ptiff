...
// Flat single-image model; compression selects the Zarr chunk compressor (None/zstd/zlib).
StorageModel makeImageModel(std::string_view compression) {
    StorageModel img;
    ...
    img.setField("compression", std::string{compression});
    return img;
}
...
std::unique_ptr<ptiff::io::StorageBackend> makeBackend() {
    auto backend = ptiff::io::BackendFactory::instance().create("zarr");
...
TEST_CASE("ZarrBackend::name and capabilities", "[zarr-backend]") {
    auto backend = makeBackend();
    const auto caps = backend->capabilities();
    REQUIRE(caps.supportsTiling);
    REQUIRE(caps.supportsStreaming);
    REQUIRE_FALSE(caps.supportsRandomAccess);
    REQUIRE_FALSE(caps.supportsCloudStreaming);
}

TEST_CASE("BackendFactory::create(\"zarr\") returns the registered backend", "[zarr-backend]") {
    auto backend = ptiff::io::BackendFactory::instance().create("zarr");
    REQUIRE(backend.has_value());
    REQUIRE((*backend)->name() == "zarr");
}

TEST_CASE("Zarr serializeModel -> deserializeModel round-trips the image model", "[zarr-backend]") {
    auto backend = makeBackend();
    auto model = makeImageModel("zstd");
    ...
    REQUIRE(parsed->children()[0].field("compression").value() == "zstd");
}

TEST_CASE("Zarr backend round-trips unencrypted chunks (compression=None)", "[zarr-backend]") {
    auto backend = makeBackend();
    auto model = makeImageModel("None"); // 32x32, 16x16 tiles -> 2x2 grid, raw chunks
...
TEST_CASE("Zarr backend round-trips compressed chunks (zstd and zlib)", "[zarr-backend]") {
    for (const char* compression : {"zstd", "zlib"}) {
        auto backend = makeBackend();
        auto model = makeImageModel(compression);
